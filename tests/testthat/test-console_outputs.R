...
test_that('Check print.xpose_data returns a proper message', {
  expect_output(print(xpdb_ex_pk), print_text, fixed=TRUE)
  expect_output(print(xpdb_modified), print_text_modified, fixed=TRUE)
})

test_that('Check prm_table returns a proper message', {
  expect_output(prm_table(xpdb_ex_pk, .problem = 1), prm_text_1, fixed=TRUE)
  expect_output(prm_table(xpdb_ex_pk, transform = FALSE), prm_text_2, fixed=TRUE)
})

test_that('Check summary.xpose_data returns a proper message', {
  expect_output(summary(xpdb_ex_pk), regexp=summary_text, fixed=TRUE)
})

test_that('Check list_vars returns a proper message', {
  expect_output(list_vars(xpdb_ex_pk, .problem = 1), regexp=vars_text, fixed=TRUE)
})
