...
test_that('summary is properly created with the appropriate information', {
  ...
  expect_equal(sum_out(sum_method(model = tibble::tibble(problem = 1L, level = 1L, subroutine = 'est', 
                                                         code = 'METH=0', comment = ''), software), 1),
               c('method', 'fo')
  )
...
test_that('summary default summary is returned for missing information', {
  ...
  expect_equal(sum_out(sum_covtime(model = tibble::tibble(problem = 1L, level = 1L, subroutine = 'lst', 
