...
```{r demo get_code_raw}
code <- get_code(xpdb, parse = FALSE)
code
```
...
The `get_source_files()` function can be used to access the name, full path, and md5 checksum of all the files that were used to build the xpose database.

```{r demo get_source_files}
run_source <- get_source_files(xpdb)
run_source
```
